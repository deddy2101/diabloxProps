...
byte MFRC522::readFromRegister(byte addr) {
  byte val;
  digitalWrite(_sad, LOW);
  rfidSPI->transfer(((addr<<1)&0x7E) | 0x80);
  val =rfidSPI->transfer(0x00);
  digitalWrite(_sad, HIGH);
  return val;
}
 ...
 */
/**************************************************************************/
void MFRC522::begin(SPIClass *spi) {
  rfidSPI = spi;
  //rfidSPI.begin(18, 16, 17, 40); // Init SPI bus the pins are connected to
  digitalWrite(_sad, HIGH);
  ...
  writeToRegister(RFCfgReg, (0x07<<4)); // TxGain = 0x07: 48dB
